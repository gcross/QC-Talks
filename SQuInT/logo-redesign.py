...
def logo(
    ...
    dot_color=(COLOR,Color(3/4,1/4,0,0)),
...
def animate_logo():
    ...
    parallel()
    dt=1
    smooth(7*dt/8,my_logo.dot_radius,0.15,e=-2)
    serial()
    wait(dt/4)
    linear(1*dt/8,my_logo.dot_color,Color(1,1,1,1))#,e=-1)
    linear(3*dt/8,my_logo.dot_color,Color(3/4,1/4,0,1))#,e=-1)
    end()
    serial()
    wait(3*dt/4)
    dt=dt/3
