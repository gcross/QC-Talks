dnl @(#) $Header: /home/cvsroot/users/nspring/adt/tcpdump.m4,v 1.6 2005/05/26 23:57:12 nspring Exp $ (LBL)
dnl
...
dnl nspring modified 2005/04/03
AC_DEFUN([AC_STRUCT_SA_STORAGE], [
	AC_MSG_CHECKING(for sockaddr_storage)
	AC_CACHE_VAL(ac_cv_struct_sa_storage,
	AC_TRY_COMPILE([
#		include <sys/types.h>
#		include <sys/socket.h>],
		[struct sockaddr_storage s],
		ac_cv_struct_sa_storage=yes,
		ac_cv_struct_sa_storage=no))
	AC_MSG_RESULT($ac_cv_struct_sa_storage)
	if test $ac_cv_struct_sa_storage = yes; then
		AC_DEFINE(HAVE_SOCKADDR_STORAGE, [], [if sockaddr storage is present])
	fi
])
